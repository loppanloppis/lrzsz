...
#include <stdint.h> //added by PL 2025-07-23
#include <string.h> //also added
#include <fcntl.h> //and this
#include <unistd.h> //also this
#include <errno.h>  //also
...
/*static void usage1 __P ((int exitcode));*/ 	/* commented out on 2025-07-23, PL */
...
#  endif
	...
	/* SklaffKOM don't play well with this check, so we remove it */
	/*
	if (getuid()!=geteuid()) {
		error(1,0,
	...
	*/
	...
	/* We go on pretending everything is normal */
	zsendline_init();
...
send_pseudo(const char *name, const char *data)
{
	...
	;
			vstringf (_ ("send_pseudo %s: cannot open tmpfile %s: %s"),
					 name, tmp, strerror (errno));  /* fixed on 2025-07-23, PL */
			free(tmp);
			vstring ("\r\n");
			return 1;
		}
...
zsendfdata (struct zm_fileinfo *zi)
{
	static int c;
	int newcnt;
	(void)newcnt;  /* silence unused-but-set-variable warning - 2025-07-23, PL */
