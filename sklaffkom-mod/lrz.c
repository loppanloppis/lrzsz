...
*/
/* New includes PL 2025-07-23 */
#include <stdbool.h>   // for 'bool'
#include <stdint.h>    // for 'uintmax_t'
#include <string.h>    // for memcpy, memmove, strcmp, strcpy, strlen, strpbrk, strdup
...
//static int make_dirs __P ((char *pathname)); /* We don't need this PL 2025 */
...
main(int argc, char *argv[])
{
	...
	textdomain (PACKAGE);
        ...
        // parse_long_options (argc, argv, show_version, usage1); Replaced 2025-07-23, PL
	//parse_long_options (argc, argv, program_name, PACKAGE, VERSION); Not good either
        parse_long_options(argc, argv, program_name, PACKAGE, VERSION, false);
...
#  endif
	...
	/* We don't need this for SklaffKOM so we comment it out */
	/*
	if (getuid()!=geteuid()) {
		error(1,0,
	...
	*/
	...
	/* And continue with initialization, PL 2025-07-23 */
...
procheader(char *name, struct zm_fileinfo *zi)
{
	...
	if (Verbose>2) {
		vstringf(_("zmanag=%zu, Lzmanag=%d\n"), (size_t)zmanag, Lzmanag);
		vstringf(_("zconv=%d\n"),zconv);
	}
...
closeit(struct zm_fileinfo *zi)
{
	...
	if (zi->modtime) {
#ifdef HAVE_STRUCT_UTIMBUF
        {
            ...
            ubuf.modtime = zi->modtime; /* or another appropriate value */
            utime(Pathname, &ubuf);
        }
#else
        {
